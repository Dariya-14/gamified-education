...
# --- ФУНКЦИИ АВТОРИЗАЦИИ И НАВИГАЦИИ ---
...
def login(request, **kwargs):
    if request.user.is_authenticated:
        # ИСПРАВЛЕНО: Добавлен префикс 'course:'
...
# --- НОВАЯ ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ДИАГНОСТИКИ ---
def handle_diagnostic_redirect(request):
    """
    Проверяет, является ли пользователь студентом, и требует ли он прохождения диагностики.
    Возвращает объект redirect, если нужно перенаправить, иначе - None.
    """
    
    # 1. Сначала пытаемся получить профиль студента через прямой запрос (надежный способ)
    try:
        student = Student.objects.get(user_id=request.user.id)
    except ObjectDoesNotExist:
        # Пользователь вошел, но он не студент (может быть, преподаватель)
        return None 

    # 2. Проверка: Диагностика уже пройдена?
    if student.diagnostic_completed:
        return None  # Диагностика пройдена, показываем обычный список классов.

    # 3. Поиск диагностического задания
    diagnostic_assignment = Assignment.objects.filter(is_diagnostic=True).first()

    if not diagnostic_assignment:
        return None  # Нет активного диагностического задания.

    # 4. Поиск конкретной задачи для теста
    task_for_diagnostic = AssignmentTask.objects.filter(
        assignment=diagnostic_assignment
    ).order_by('id').first()

    if task_for_diagnostic:
        # 5. Гарантируем, что студент записан на этот курс (для предотвращения ошибок)
        course_class_for_diag = task_for_diagnostic.course_class
        Enrollment.objects.get_or_create(
            student=student,
            course_class=course_class_for_diag,
            defaults={'lost_lives': 0}
        )

        # 6. Перенаправляем на страницу теста
        # ИСПРАВЛЕНО: Добавлен префикс 'course:' для app_name
        return redirect('course:show_diagnostic_test', 
                        course_code=course_class_for_diag.course.code, 
                        class_code=course_class_for_diag.code, 
                        assignment_task_id=task_for_diagnostic.id)
    
    return None # Если что-то не нашлось


# --- ОБНОВЛЕННАЯ ФУНКЦИЯ CLASSES ---

@login_required(login_url='/login/')
def classes(request):
    
    # 1. ПЕРВАЯ ПРОВЕРКА: Нужно ли перенаправить на диагностику?
    redirect_response = handle_diagnostic_redirect(request)
    if redirect_response:
        return redirect_response # Если нужно, то перенаправляем отсюда и останавливаемся.
        
    # 2. ОПРЕДЕЛЕНИЕ КЛАССОВ ДЛЯ ОТОБРАЖЕНИЯ (Обычная логика)
    
    if hasattr(request.user, 'student'):
        # Если студент, то ищем все его записи (Enrollments)
        all_classes = CourseClass.objects.filter(
            enrollment__student=request.user.student
        )
    elif hasattr(request.user, 'instructor'):
        # Если преподаватель, ищем классы, которые он ведет
        all_classes = CourseClass.objects.filter(
            classinstructor__instructor=request.user.instructor
        )
    else:
        all_classes = CourseClass.objects.none()
    
    # 3. ЛОГИКА ОТОБРАЖЕНИЯ (Если только один класс, перенаправляем туда)
    if len(all_classes) == 1 and not request.user.is_staff:
        course_class = all_classes[0]
        # ИСПРАВЛЕНО: Используем redirect с 'course:home'
        return redirect('course:home', course_code=course_class.course.code, class_code=course_class.code)
    else:
        # 4. Отображение списка классов
        return render(
            request,
            'course/classes.html',
            {
...
# --- ФУНКЦИИ ДЛЯ ДИАГНОСТИКИ (SHOW И SUBMIT) ---

@login_required(login_url='/login/')
def show_diagnostic_test(request, course_code, class_code, assignment_task_id):
    """Отображает форму диагностического теста (для нового студента)."""
    try:
        enrollment = get_enrollment(request.user.id, course_code, class_code)
        course_class = enrollment.course_class
        assignment_task = AssignmentTask.objects.get(
            pk=int(assignment_task_id), 
            course_class=course_class
        )
    except ObjectDoesNotExist:
        raise Http404(_("Задание или Запись на курс не найдены"))

    # ИСПРАВЛЕНО: Проверяем Grade, а не флаг
    if Grade.objects.filter(enrollment=enrollment, assignment_task=assignment_task).exists():
         return redirect('course:classes') 

    # Получаем вопросы для этого задания (Task)
    questions = Question.objects.filter(task=assignment_task.task).all()
    
    # Проверяем, что есть вопросы
    if not questions.exists():
         return render(request, 'course/diagnostic_test_form.html', 
                       {'assignment_task': assignment_task, 'questions': [], 'active_tab': 'assignments'})


    return render(
        request,
        'course/diagnostic_test_form.html',
        {
...
@login_required(login_url='/login/')
def submit_diagnostic_test(request, course_code, class_code, assignment_task_id):
    """Обрабатывает ответы на диагностический тест и обновляет статус студента."""
    if request.method != 'POST':
        # ИСПРАВЛЕНО: Добавлен префикс 'course:'
        return redirect('course:show_diagnostic_test', course_code=course_code, class_code=class_code, assignment_task_id=assignment_task_id)
    ...
    # ИСПРАВЛЕНО: Проверка на повторную сдачу (во избежание IntegrityError)
    if Grade.objects.filter(enrollment=enrollment, assignment_task=assignment_task).exists():
        print(f"Студент {enrollment.student.full_name} попытался сдать диагностику повторно.")
        return redirect('course:classes')
    
    # --- ЛОГИКА ПРОВЕРКИ ОТВЕТОВ И СОЗДАНИЯ GRADE ---
    questions = Question.objects.filter(task=assignment_task.task).all()
    ...
    for question in questions:
        ...
        if submitted_answer and submitted_answer.strip().lower() == question.correct_answer.lower():
            correct_count += 1
            
    # 2. Вычисляем процент (Score для Grade)
    score_percent = correct_count / total_questions if total_questions > 0 else 0.0

    # Создаем запись Grade (это начисляет XP)
    ...
    # 3. Обновление статуса диагностики
    student = enrollment.student
    student.diagnostic_completed = True
    student.save()
    
    # Перенаправление на страницу классов после завершения диагностики
    # ИСПРАВЛЕНО: Добавлен префикс 'course:'
    return redirect('course:classes') 
...
def get_tasks_data(assignment, enrollment):
    ...
    for assignment_task in assignment.ordered_assignment_tasks(enrollment.course_class):
        ...
        task_data = {}
        task_data['id'] = assignment_task.id
